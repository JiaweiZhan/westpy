...
def write_pdep(pdep_val: np.ndarray, fname: str):
...
class qe_io(base_io):
    def __init__(self, wfc_fname=None, wstat_folder=None):
        super().__init__()
    ...
    def read_wfc(self, fname: str, read_evc: bool = True):
        self.fname = fname
        with open(self.fname, "rb") as f:
            ...
            if read_evc:
                self.evc = np.zeros(
                    (self.nbnd, self.npol * self.igwx), dtype="complex128"
                )

                f.seek(8, 1)
                for i in range(self.nbnd):
                    self.evc[i, :] = np.fromfile(
                        f, dtype="complex128", count=self.npol * self.igwx
                    )
                    f.seek(8, 1)
        volume_resiprocal = np.abs(np.dot(np.cross(self.b1, self.b2), self.b3))
        self.omega = np.power(2 * np.pi, 3) / (volume_resiprocal)
        self.a1 = (
            2 * np.pi * (np.cross(self.b2, self.b3) / volume_resiprocal)
        ).tolist()
        self.a2 = (
            2 * np.pi * (np.cross(self.b3, self.b1) / volume_resiprocal)
        ).tolist()
        self.a3 = (
            2 * np.pi * (np.cross(self.b1, self.b2) / volume_resiprocal)
        ).tolist()
    ...
    def read_wstat(
        ...
        self.pdepeig = np.array(
            data["exec"]["davitr"][-1]["ev"], dtype="f8"
        )  # -1 identifies the last iteration
    ...
    def write_wstat(
        ...
        with tempfile.TemporaryDirectory(dir=outFolder) as tempdir:
            pdepvec_fname = os.path.join(tempdir, "pdepvec.npy")
            np.save(pdepvec_fname, pdepvec)

            args = [
                (i, fname, pdepvec_fname, outFolder) for i, fname in enumerate(fnames)
            ]
            with Pool() as pool:
                pool.starmap(write_wstat_helper, args)
